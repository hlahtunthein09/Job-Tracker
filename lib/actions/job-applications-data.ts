...
interface jobApplicationsDataProps {
    company: string;
    position: string;
    location?: string;
    salary?: string;
...
export default async function createJobApplicationsData(data: jobApplicationsDataProps)
{
    // Check again for use is autherized or not
    ...
    // connect MongoDB server again for making sure
    await connectDB();

    const { company, position, location, salary, jobUrl, tags, notes, description, boardId, columnId } = data;

    // check if all required fields and Ids are provided
    ...
    // finally creating job application
    ...
    // find jobApplications array inside Column and update with new jobApplication
...
export async function deleteJobApplication(id: string)
{
    ...
    // first remove from the column
    await Column.findByIdAndUpdate(jobApplication.columnId, {
        $pull: { jobApplications: id }
    });

    // Then, remove from the jobApplication collection
