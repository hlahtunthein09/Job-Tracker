...
import CreateJobApplicationDialog from "./create-jobapp-dialog";
...
function DroppableColumn(
    {column, config, boardId, sortedColumns, onDelete} : {column: Column, config: ColConfig, boardId: string, sortedColumns: Column[], onDelete: (columnId: string) => void}
){
...
{   
    ...
    const { columns, moveJob, handleDeleteColumn } = useBoards(board);
    ...
    async function handleDragEnd(event: DragEndEvent) {
        ...
        if(targetColumn)
        {
            ...
            const jobsInTarget = targetColumn.jobApplications.filter((job) => job._id !== activeId).sort((a, b) => a.order - b.order) || [];
            ...
            <div className="space-y-4">
                <div className="flex gap-4 overflow-x-auto pb-4">
                    {sortedColumns.map((col, key) => {
                        ...
                        return(
                            <DroppableColumn key={key} column={col} config = {config} boardId = {board._id} sortedColumns = {sortedColumns} onDelete={handleDeleteColumn}    />
                        )
                    })}
                </div>
            </div>
